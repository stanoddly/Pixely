...
internal readonly record struct PathEdge(int Destination, float Cost);

internal interface IIndexedPathGraph
{
    int NodeCount { get; }
...
internal interface IIndexedPathHeuristic
{
    float EstimateCost(int origin, int destination);
}

internal sealed class IndexedPathSearch<TGraph> where TGraph : struct, IIndexedPathGraph
{
