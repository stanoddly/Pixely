...
internal sealed class InputAutomation
{
    ...
    // window never leak into another. SDL hands out small device ids, so the synthetic ones count down from the top.
    private static SDL_MouseID VirtualMouseId(ViewScope viewScope) => (SDL_MouseID)(uint.MaxValue - (uint)viewScope.Value);
    private static SDL_KeyboardID VirtualKeyboardId(ViewScope viewScope) => (SDL_KeyboardID)(uint.MaxValue - (uint)viewScope.Value);
