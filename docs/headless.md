...
A headless app reads command lines from the process's standard input (on the desktop only; headless mode is not available in the browser, see below) and runs each one on the frame loop at `UpdateOrders.Input`, after the frame's real events and before the game's updatables. Blank lines and lines starting with `#` are ignored. A malformed line throws `FormatException` out of the frame loop, so a bad script ends the app. Lines queued before a frame starts run in that frame; a chord written in one go usually lands in one frame but may split across two. An exception thrown by an input handler propagates out of the frame loop, the same as for real input.
