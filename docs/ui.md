...
The selector runs every frame against the context the provider built, so the provider decides what the texture is. It receives a copy of the context, since a `Func` takes its argument by value, so it reads from the context and records nothing on its command buffer. A menu that shades the UI has the provider hand out an offscreen texture and a renderer after `RenderOrders.Ui` that samples it and draws the result over the colour target; a stage that does not can have the provider hand out the swapchain texture again, and nothing else changes. The selected texture must be the size `GetColorTargetSize` reports and in the window's colour format, since the tree was laid out for that size and the present pipeline is built for that format. The size is the provider's answer, not the selected texture's: the tree is laid out in the update phase, where no context exists to select from, so `GetColorTargetSize` is asked and the renderer refuses to present into a selected texture of another size, the same as for the colour target. A UI texture sized differently from the colour target, such as a window-resolution UI over a low-resolution scene target, is not supported by the selector alone. `clearTarget` applies to whatever is selected and clears to opaque grey, so a game that selects its own texture clears it to transparent itself, in the provider that owns it, and leaves `clearTarget` false.
